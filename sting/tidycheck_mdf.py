...
def main():
    ...
    for line in f:
        if line.startswith("Track"):
            this_track = int(line.split()[1])
        elif line.startswith("Point"):
            split = line.split()
            frameno = int(float(split[-2]))
            tracks_by_point.setdefault(frameno, []).append(this_track)
            n_points_by_track[this_track] += 1

    mode_n_points = Counter(n_points_by_track.values()).most_common(1)[0][0]
    # these are the right length
    goldilocks_tracks = [track for track, n_points
                         in n_points_by_track.most_common()
                         if n_points == mode_n_points]
    ...
    for line in f:
        ...
        if line.startswith("Track"):
            this_track = int(line.split()[1])
            if this_track in goldilocks_tracks:
                print line
        elif line.startswith("Point"):
